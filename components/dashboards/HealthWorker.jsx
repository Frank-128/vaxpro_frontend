    ...
    <div>
      {loggedUser && (
        <div className={"h-screen flex flex-col gap-6 md:gap-3"}>
          <div
            className={
              "flex justify-between flex-col md:flex-row md:gap-3 gap-2"
            }
          >
            <div
              className={
                "bg-blue-500/15 p-1 md:p-2 rounded flex gap-2 items-center md:w-3/4 w-full"
              }
            >
              <span> Welcome Back</span>
              {loggedUser.role.account_type === "health_worker" ? (
                <>
                  <span className={"text-xl font-monte-1 text-blue-800"}>
                    {loggedUser?.health_workers[0]?.first_name}
                  </span>
                  <span className={"text-xl font-monte-1 text-blue-800"}>
                    {loggedUser?.health_workers[0]?.last_name}{" "}
                  </span>
                </>
              ) : (
                <span className={"text-xl font-monte-1 text-blue-800"}>
                  {loggedUser?.facilities?.facility_name} Branch Manager
                </span>
              )}
              <span>!!</span>
            </div>
            <Button
              className={
                "md:w-1/4 w-fit self-end rounded-[0.25rem] bg-blue-900 flex justify-center items-center gap-1 md:gap-3"
              }
              onClick={() => {
          ...
          <div className={"flex flex-col md:flex-row w-full h-[35rem] gap-2"}>
            <div className={"flex w-full md:w-4/6 flex-col"}>
              <div
                className={
                  "md:flex grid grid-cols-2 w-full h-[15rem] md:h-[20%] gap-2"
                }
              >
                {members.map(
                  ({ bg, name, textColor, valueColor, value }, index) => (
                    <div
                      key={index}
                      className={`${bg} 
                    ...
                    >
                      ...
                      <span
                        className={`${valueColor} font-monte-1 font-extrabold`}
                      >
                        {value}
                      </span>
                    </div>
              ...
              <div
                className={
                  "bg-[#BF40BF]/10 h-[80%] rounded-md flex justify-center items-center "
                }
              >
            ...
            <div
              className={
                "bg-black/20 rounded-lg w-full md:w-2/6 flex items-center justify-center"
              }
            >
