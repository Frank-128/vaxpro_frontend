...
const ParentGuardianForm = ({register, setValue, errors, control, errTouched, loading}) => {
    const [isFocused, setIsFocused] = useState(false);
    const [availableParent, setAvailableParent] = useState({
        status: false, parent: null,
    });
    ...
    const handleNidaRes = (res) => {
        if (res === 1) {
            ...
            const {nida_id, firstname, middlename, lastname, user} = availableParent.parent;
            ...
            <div className="flex flex-col md:gap-6 gap-4 w-full pt-6">
                <div className={'flex flex-col md:flex-row gap-4 md:gap-3 w-full'}>
                    <div className="relative w-[95%] sm:w-[85%] md:[70%] lg:w-[40%] flex flex-col gap-3">
                        <Input
                            ...
                            {...register("nida_id", {
                                required: "This field is required", maxLength: {
                                    value: 20, message: "Nida number can not have more than 20 numbers",
                                }, minLength: {
                                    value: 20, message: "Nida number can not have less than 20 numbers",
                                }, pattern: {
                        ...
                        {availableParent.status && <Card
                            className="absolute top-11 p-4 cursor-pointer w-full bg-gray-50 shadow-xl border-gray-200 border-[0.4px] z-50">
                            <div className={'flex flex-col gap-1 w-full'}>
                                <div
                                    className="font-bold text-xs ">{availableParent.parent?.firstname + " " + availableParent.parent?.lastname}</div>
                    ...
                    <div className={'w-[95%] sm:w-[85%] md:[70%] lg:w-[40%] flex flex-col gap-3'}>
                        <Input
                            label="First Name"
              ...
              >
                +255
                            ...
                            <Input
                                ...
                                {...register("contact", {
                                    onBlur: () => setIsFocused(false), required: "This field is required", maxLength: {
                        ...
                        {errors.contact && (<p className="text-red-900 text-xs px-2">
                    ...
                    <Controller
                        ...
                        render={({field: {onBlur, value, onChange}, fieldState: {error}}) => (
                            <div className={'w-[95%] sm:w-[85%] md:[70%] lg:w-[40%] flex flex-col gap-3'}>
                                ...
                                >
                                    <Option className={'text-black font-monte-1'} value="Mother">Mother</Option>
                                    <Option className={'text-black font-monte-1'} value="Father">Father</Option>
                                    <Option className={'text-black font-monte-1'} value="Relative">Relative</Option>
                                </Select>
                                {error && (<p className="text-red-900 text-xs px-2">
                                        {error.message}
                                    </p>)}{" "}
                            </div>)}
                    />
