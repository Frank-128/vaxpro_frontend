...
const Filter = ({setAllChildren}) => {
    ...
    const [filterValues, setFilterValues] = useState(loggedInUser?.role?.account_type === "regional" ? {
        region: loggedInUser.region_id, district: null, year: null, vaccine: null, gender: null,
    } : loggedInUser?.role?.account_type === "district" ? {
        region: loggedInUser.region_id, district: loggedInUser.district_id, year: null, vaccine: null, gender: null,
    } : {
        region: null, district: null, year: null, vaccine: null, gender: null,
    });
    ...
    const filter_array = [{
        option_type: "region", options: regions, authorized_accounts: ["ministry"],
    }, {
        option_type: "district", options: districts, authorized_accounts: ["ministry", "regional"],
    }, {
        option_type: "gender", options: ["Male", "Female"], authorized_accounts: ["ministry", "regional", "district"],
    }, {
        option_type: "year",
        options: [2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024,],
        authorized_accounts: ["ministry", "regional", "district"],
    }, {
        option_type: "vaccine", options: vaccines, authorized_accounts: ["ministry", "regional", "district"],
    },];
    ...
    return (<div
            className={"flex justify-around md:justify-end gap-2 overflow-x-scroll pb-2 md:mb-4"}
        >
            {filter_array
                .filter((item) => item.authorized_accounts.includes(loggedInUser?.role?.account_type))
                .map(({options, option_type}, index) => (<select
                        key={index}
                        value={filterValues[option_type] == null ? "" : filterValues[option_type]}
                        onChange={(e) => handleSelect(option_type, e.target.value === "all" ? null : e.target.value)}
                    ...
                    >
                        {option_type === "region" ? (<>
                                ...
                                {districts?.length !== 0 ? (options?.map(({district_name, id}, index) => (
                                        <option value={id} key={index} className={""}>
                                            {district_name}
                                ...
                                {options?.map((year, index) => (<option value={year} key={index} className={""}>
                                        {year}
                                    </option>))}
                            </>)}
                    </select>))}
            <Button
                ...
                className={"bg-transparent shadow-none hover:shadow-none w-28 h-8 p-2 text-xs border border-black text-black rounded-[0.25rem] flex justify-center items-center gap-2"}
            >
                <span> Reset all</span>
            </Button>
        </div>);
