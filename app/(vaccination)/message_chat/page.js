...
const ChatContainer = () => {
  ...
  useEffect(() => {
    if (echo) {
      echo.private(`chat.${loggedInUser.id}`).listen("MessageSent", (event) => {
        ...
        setChatHeaders((prev)=>[...prev,event.sender]);
  ...
  return (
    <div className="flex  h-[calc(100vh-80px)]">
      <div className="basis-1/6 bg-gray-200 border-r-[0.2px] border-gray-400">
        {
          chatHeaders.length !== 0 && 
          chatHeaders.map((item,key)=>{
            item
          })
          }
      </div>
      <div className="flex flex-col bg-gray-100 basis-5/6">
    ...
    <>
      <div className={`${is_sender ? "self-end" : "self-start"}  w-fit`}>
        ...
        <div class="w-max grid">
          <div
            class={`px-3.5 py-2 bg-white rounded-3xl ${
              is_sender ? "rounded-tl-none" : "rounded-br-none"
            } justify-start  items-center gap-3 inline-flex`}
          >
      ...
      {messages.length == 0
        ...
        : messages?.map((message, index) => (
            <MessageItem
              ...
              is_sender={message.sender.id == 102}
            />
          ))}
    </div>
  );
  ...
  const sendMessage = async () => {
    if (message.trim()) {
      const res = await axios.post(
        "send_message",
        { sender: loggedInUser.id, receiver: recipient, message },
        {
          headers: {
