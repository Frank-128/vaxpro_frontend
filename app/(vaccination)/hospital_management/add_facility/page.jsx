    ...
    try {
      const fetchWards = async () => {
        const res = await axios.get(
          "http://localhost:8000/api/district_wards/13"
        );
        setWards(res.data);
      };
  ...
  return (
    <div className="w-full h-fit py-4 px-8 flex flex-col gap-4">
      ...
      <form onSubmit={handleSubmit(submitForm)}>
          ...
          <Card className={activeStep !== 1? "hidden" :"flex flex-col gap-8 p-5"}>
            <Input label="Contacts" {...register("contacts")} />
            <Input label="Role" defaultValue={"Role - Branch admin"} disabled />
          </Card>
       
          <Card className={activeStep !== 2? "hidden" :"flex flex-col gap-8 p-5"}>
