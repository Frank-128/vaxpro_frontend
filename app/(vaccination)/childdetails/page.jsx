...
export default function TeamSection12() {
    ...
    const handleClickOpenDateViewer = () => {
        {
            axios
                .post(`/getAllChildSchedules`, {
                    vaccines: allVaccines, date: birth_date,
                })
                .then((res) => {
                    if (res.data) {
    ...
    return (<section className="min-h-screen py-8 px-8 lg:py-2">
            <div className="container mx-auto">
                ...
                <div
                    className="!text-2xl lg:!text-3xl shadow-lg text-black bg-[#e8dcc5] p-6 self-left font-bold flex rounded-r-full mb-6">
                    {childData.map((child, index) => (<div key={index}>
                            {child.firstname} {child.middlename} {child.surname}
                        </div>))}
                </div>

                <div className="grid w-full grid-cols-1 gap-1  mb-10">
                    {childData.map((props, key) => (<TeamCard key={key} {...props} handleGenerate={handleGenerate}/>))}
                </div>
